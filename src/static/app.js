  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (!refreshed) {
          messageDiv.textContent = "Signed up successfully, but failed to refresh activities. Please refresh the page.";
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const refreshed = await fetchActivities();
        if (!refreshed) {
          messageDiv.textContent = "Participant removed, but failed to refresh activities. Please refresh the page.";
          messageDiv.className = "info";
        }
      } else {
        messageDiv.textContent = result.detail || "An error occurred";
